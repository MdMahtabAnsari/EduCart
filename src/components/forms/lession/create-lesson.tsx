...
import { createLessionSchemaFrontEnd, CreateLessionSchemaFrontEnd, CreateLessionSchemaBackEnd } from "@/lib/schema/lession";
...
export function CreateLesson({ courseId, sectionId }: { courseId: string, sectionId: string }) {
    const form = useForm<CreateLessionSchemaFrontEnd>({
        resolver: zodResolver(createLessionSchemaFrontEnd),
        defaultValues: {
            courseId: courseId,
            sectionId: sectionId,
            title: '',
    ...
    const createLesson = api.teacher.lesson.createLesson.useMutation();

    const onSubmit = async (data: CreateLessionSchemaFrontEnd) => {
        toast.promise(
            createLesson.mutateAsync(data as CreateLessionSchemaBackEnd),
            {
    ...
    return (
        <Card className="w-full max-w-2xl mx-auto">
            <CardHeader>
                <CardTitle className="text-2xl font-bold">Create New Lesson</CardTitle>
